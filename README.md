...
Локальный MCP для чтения, редактирования и создания макетов с нуля через Figma Plugin API. Версия 0.7.28: переносимый пакет с открытыми исходниками runtime и плагина, профилем Gravity UI по умолчанию и дополнительным shadcn/ui, правилами проекта и сценарием style guide → компоненты → экраны.
...
Передайте `dist/figma-local-design-skill-0.7.28.zip`. Получателю достаточно установить папку `figma-local-design` как скилл и написать агенту:
