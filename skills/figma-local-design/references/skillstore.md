...
- Include `SKILL.md`, `version.json`, `agents/`, `references/`, `scripts/install.mjs` and both runtime assets (`runtime-payload.json.gz` and `runtime-release.json`). The compressed payload contains the MCP implementation; preserve its SHA256 metadata and bundled dependency licenses. Rebuild it using the package build when the runtime changes.
