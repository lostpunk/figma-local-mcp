...
// src/onboarding.mjs
import { lstat as lstat2, readFile as readFile3, realpath as realpath2, stat as stat2, writeFile } from "node:fs/promises";
...
async function readProjectFile(root, name2) {
  const path = await realpath(resolve(root, name2));
  const rel = relative(root, path);
  if (rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel)) throw new Error(`File escapes the selected project: ${name2}`);
  const info = await stat(path);
  if (!info.isFile() || info.size > 65536) throw new Error(`Expected a regular file of at most 64 KiB: ${name2}`);
...
async function loadProjectRules(projectRoot) {
  ...
  let raw;
  try {
    raw = await readProjectFile(root, ".figma-design.json");
  } catch (error) {
    if (error.code === "ENOENT") {
      const { lstat: lstat3 } = await import("node:fs/promises");
      try {
        await lstat3(resolve(root, ".figma-design.json"));
      } catch (missing) {
        if (missing.code === "ENOENT") return { status: "missing", config: null, ruleFiles: [], guidePatch: null };
        throw missing;
      }
    }
    throw error;
  ...
  try {
    ...
    throw new Error(".figma-design.json is not valid JSON");
  }
  ...
  const ruleFiles = [];
  for (const name2 of new Set([config.profileFile, ...config.rulesFiles].filter(Boolean))) {
    ruleFiles.push({ path: name2, text: await readProjectFile(root, name2) });
  }
  const foundation = config.foundation;
  const guidePatch = foundation ? Object.fromEntries(Object.entries({
    collectionId: foundation.collectionId,
    colors: foundation.colors,
    typography: foundation.typography,
    spacing: foundation.spacing?.map((value) => ({ name: String(value), value })),
    radii: foundation.radii
  }).filter(([, value]) => value !== void 0)) : null;
  return {
    status: "valid",
    config,
    ruleFiles,
...
var distributionSchema = external_exports.object({
  schemaVersion: external_exports.literal(1),
  id: external_exports.string().regex(/^[a-z0-9-]{1,80}$/),
  label: external_exports.string().min(1).max(200),
  welcome: external_exports.string().min(1).max(2e3),
...
async function loadDistribution(packageRoot = dirname(dirname(fileURLToPath(import.meta.url)))) {
  ...
  return distributionSchema.parse(JSON.parse(await readFile2(path, "utf8")));
}
...
function configFromAnswers(raw, projectName, distribution = null) {
  ...
  if (answers.designSystem === "distribution") {
    ...
    const config2 = configFromAnswers({ ...answers, designSystem: "custom", libraryName: policy.defaults.library.name }, projectName);
    return projectConfigSchema.parse({ ...config2, ...policy.defaults, rules: [...policy.defaults.rules, ...answers.rules ?? []] });
  }
  if (answers.designSystem === "later") return projectConfigSchema.parse({ onboarding: { status: "deferred", designSystem: "later" } });
  const config = {
    schemaVersion: 1,
    onboarding: { status: "configured", designSystem: answers.designSystem },
    profile: answers.designSystem === "shadcn-ui" ? "shadcn-ui" : "custom",
    foundation: { name: answers.name ?? projectName },
    rules: answers.rules ?? []
  };
  if (answers.designSystem === "existing") {
    ...
    config.library = {
      name: answers.designSystem === "shadcn-ui" ? "shadcn/ui" : answers.designSystem === "material" ? "Material Design" : answers.libraryName,
      mode: "reference",
      components: {}
    };
  }
...
function projectQuestions(distribution = null) {
  if (!distribution) return questions;
  return [{ ...questions[0], defaultValue: "distribution", options: [
    { value: "distribution", label: `${distribution.label} \u2014 \u043F\u043E \u0443\u043C\u043E\u043B\u0447\u0430\u043D\u0438\u044E` },
    ...questions[0].options
  ] }, ...questions.slice(1)];
}
async function collectAnswers(ask, projectName, distribution = null) {
  const choose = async (prompt, options, defaultValue) => {
    while (true) {
      const response = (await ask(`${prompt}
${options.map(([value2, text], i) => `${i + 1}. ${text}`).join("\n")}
\u0412\u044B\u0431\u043E\u0440${defaultValue ? ` [${defaultValue}]` : ""}: `)).trim();
  ...
  if (designSystem === "custom") {
    do {
      answers.libraryName = (await ask("\u041D\u0430\u0437\u0432\u0430\u043D\u0438\u0435 \u0441\u0432\u043E\u0435\u0439 \u0434\u0438\u0437\u0430\u0439\u043D-\u0441\u0438\u0441\u0442\u0435\u043C\u044B: ")).trim();
    } while (!answers.libraryName);
  }
  ...
  while (true) {
    const color2 = (await ask("\u0426\u0432\u0435\u0442 \u0431\u0440\u0435\u043D\u0434\u0430 #RRGGBB [Enter \u2014 \u0441\u043E\u0445\u0440\u0430\u043D\u0438\u0442\u044C \u0442\u0435\u043A\u0443\u0449\u0438\u0439]: ")).trim();
    if (!color2) break;
    if (/^#[0-9a-fA-F]{6}$/.test(color2)) {
      answers.primaryColor = color2;
      break;
    }
  }
...
async function onboardProject(options = {}) {
  const distribution = options.distribution === void 0 ? await loadDistribution() : options.distribution === null ? null : distributionSchema.parse(options.distribution);
...
`);
  ...
  const root = await realpath2(projectRoot);
  if (!(await stat2(root)).isDirectory()) throw new Error("Select a project directory");
  const path = join2(root, ".figma-design.json");
  let present = false;
  try {
    await lstat2(path);
    present = true;
  ...
  if (!answers) return {
    status: "needs_input",
    project: root,
    questions: projectQuestions(distribution),
...
async function main(argv) {
  ...
  console.log(JSON.stringify(await onboardProject({
    projectRoot: options.project,
    answersFile: options.answers,
    interactive: !options.nonInteractive && !!process.stdin.isTTY && !!process.stdout.isTTY
  }), null, 2));
}
